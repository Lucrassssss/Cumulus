...
export default {
  fetch: withSupabase({ auth: ["publishable", "secret"] }, async (req, ctx) => {
    ...
    const signature = req.headers.get("stripe-signature") || req.headers.get("x-yoti-signature");
    ...
    try {
      ...
      const userId = payload.data?.object?.client_reference_id || payload.user_id || payload.userId;
