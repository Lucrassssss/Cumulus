...
1. **Row lock = mutual exclusion.** `FOR UPDATE` takes a row-level *exclusive*
...
2. **The subtle part — snapshot vs. lock re-read.** Under the default
   ...
   statement start. That alone would *not* save us: both transactions could
...
3. **Idempotency is separate from the counter.** The row lock stops *different*
   users from oversubscribing the counter. A *single* user retrying (double-tap,
...
4. **Atomic all-or-nothing.** Everything runs in the function's single implicit
   transaction. Any `RAISE` rolls back the redemption insert *and* the
   decrement *and* the user update together, and releases the lock. There is no
...
`age_verified_at`, `invited_by`, `onboarded_at`. RLS can gate *which rows* a
user updates but not *which columns*, so a `BEFORE UPDATE` trigger
